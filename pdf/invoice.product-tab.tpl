...
<table class="product" width="100%" cellpadding="4" cellspacing="0">
	...
	<tr>
		...
		<th class="product header small" width="{$layout.tax_code.width}%">&nbsp;</th>
		
		<th class="product header small" width="{$layout.unit_price_tax_excl.width}%">{l s='Unit Price' pdf='true'} <br /> {l s='(Tax excl.)' pdf='true'}</th>
		
		<th class="product header small" width="{$layout.unit_price_tax_excl.width}%">{l s='Unit Price' pdf='true'} <br /> {l s='(Tax incl.)' pdf='true'}</th>
	...
	{foreach $order_details as $order_detail}
		...
		<tr class="product {$bgcolor_class}">
			...
			<td class="product left">
				...
				{else}
					{$order_detail.product_name}
					{if $order_detail.id_category_default == 63}
					<br><small>{l s='Produit inclus uniquement dans les lots de Cartes et Livrets' pdf='true'}</small>
					{/if}
				{/if}
						...
						{if isset($customization.datas[$smarty.const._CUSTOMIZE_TEXTFIELD_]) && count($customization.datas[$smarty.const._CUSTOMIZE_TEXTFIELD_]) > 0}
							<table style="width: 100%;">
								{foreach $customization.datas[$smarty.const._CUSTOMIZE_TEXTFIELD_] as $customization_infos}
									...
									{if $customization_infos.value != '_DATE_01-01-1970'}
									<tr>
										<td >
											{$customization_infos.name|string_format:{l s='%s:' pdf='true'}} <br>
											<b>{$customization_infos.value}</b>
										</td>
