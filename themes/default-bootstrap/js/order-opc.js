...
function updatePaymentMethods(json)
{
	...
	//$('#cart_title').html(json.HOOK_TOP_PAYMENT);
	
//test si il y a une erreur
if (json.HOOK_PAYMENT.indexOf("danger") >= 0){
	
	//alert('error');
		$('#opc_payment_methods-content #HOOK_PAYMENT').html('<div class="alert alert-danger"><ol><li>'+ json.HOOK_PAYMENT +'</li></ol></div>');		
		
		$('#cart_title').append('<div class="alert alert-danger"><ol><li>'+ json.HOOK_PAYMENT +'</li></ol></div>');
	//$('#cart_title').html(json.HOOK_PAYMENT);
	...
	$.ajax({
		...
		success: function(jsonData)
		{
			if (jsonData.hasError)
			{
				...
				if(errors!=='There is no product in your cart.'){
					$.fancybox.open([
							{
								type: 'inline',
								autoScale: true,
