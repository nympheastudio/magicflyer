 ...
 *}
	...
	<script type="text/javascript">
       ...
       /*
        * text translation for ordersSend.js
        */
        var order_translation = {
            orders_successfully_sent : "{l s='Your orders were successfully sent.' mod='envoimoinscher'}",
            orders_successfully_sent_except : "{l s='Your orders were successfully sent, except these:' mod='envoimoinscher'}",
            orders_not_sent : "{l s='Orders which were not sent will be available in the Orders pending shipment table, under Invalid or Incomplete orders order type.' mod='envoimoinscher'}",
            page_refresh : "{l s='This page will automatically reload in 10 seconds.' mod='envoimoinscher'}",
            error_occured : "{l s='An error occurred whilst sending orders:' mod='envoimoinscher'}",
            please_refresh : "{l s='Please reload page.' mod='envoimoinscher'}"
        };
	</script>
	...
	<div class="bootstrap">
		...
		<div class="panel mass_order_container_mess">
		...
		<div class="panel">
			...
			<form id="orderDo" method="post" action="index.php?controller=AdminEnvoiMoinsCher&option=initOrder&token={$token|escape:'htmlall':'UTF-8'}">
				<div>
					{*{if !isset($filters) || !isset($filters.type_order) || $filters.type_order|@count > 1}
						{if !isset($filters)}
							{assign "filters" ""}
						{/if}
					{else}*}
