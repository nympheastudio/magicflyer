		...
		overrideButtonsInThePage : function(){
			...
			$(modalCartjQuerySelector).unbind('click').click(function(e){
				...
				if ($(this).attr('disabled') != 'disabled') {
					if ($('input#quantity_to_cart_'+idProduct).size() > 0 && !isNaN($('input#quantity_to_cart_'+idProduct).val())) {
						modalAjaxCart.add(idProduct, idProductAttribute, false, $(this), $('input#quantity_to_cart_'+idProduct).val());
					} else {
		...
		add : function(idProduct, idCombination, addedFromProductPage, callerElement, quantity, whishlist){
			...
			$.ajax({
				...
				success: function(jsonData)	{
					...
					let cart_qty = $('.ajax_cart_quantity').text();
					let new_qty = parseFloat(quantity) + parseFloat(cart_qty);
					console.log(parseFloat(quantity) + parseFloat(cart_qty));
					console.log( new_qty );
					
					$('.ajax_cart_no_product').text(new_qty);
					$('.ajax_cart_quantity').text(new_qty);
