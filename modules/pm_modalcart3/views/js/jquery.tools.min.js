...
(function($jqPm) {	
	...
	$jqPm.tools = $jqPm.tools || {version: '@VERSION'};
		
	// globals
	var typeRe = /\[type=([a-z]+)\]/, 
		numRe = /^-?[0-9]*(\.[0-9]+)?$/,
		dateInput = $jqPm.tools.dateinput,
	...
	v = $jqPm.tools.validator = {
		...
		localize: function(lang, messages) { 
			$jqPm.each(messages, function(key, msg)  {
		...
		localizeFn: function(key, messages) {
			v.messages[key] = v.messages[key] || {};
			$jqPm.extend(v.messages[key], messages);
		},
		...
		'default' : [
			...
			function(errs) {
				...
				$jqPm.each(errs, function(i, err) {
					...
					if (!msg) { 
						msg = $jqPm(conf.message).addClass(conf.messageClass).appendTo(document.body);
					...
					// populate messages
					$jqPm.each(err.messages, function(i, m) { 
						$jqPm("<p/>").html(m).appendTo(msg);			
					});
	...
	/* sperial selectors */
	$jqPm.each("email,url,number".split(","), function(i, key) {
		$jqPm.expr[':'][key] = function(el) {
	...
	/* 
		...
		Usage: $jqPm("input:eq(2)").oninvalid(function() { ... });
	*/
	$jqPm.fn.oninvalid = function( fn ){
    ...
    form.attr("novalidate", "novalidate");
		...
		function pushMessage(to, matcher, returnValue) {
			...
			if (returnValue === false || $jqPm.isArray(returnValue)) {
				...
				if (matches && $jqPm.isArray(returnValue)) {
					$jqPm.each(matches, function(i) {
		...
		$jqPm.extend(self, {
			...
			invalidate: function(errs, e) {
				...
				if (!e) {
					var errors = [];
					$jqPm.each(errs, function(key, val) {
...
//{{{  checkValidity() - flesh and bone of this tool
			...
			checkValidity: function(els, e) {
				...
				els.not(":radio:not(:checked)").each(function() {
					...
					var msgs = [], 
						 el = $jqPm(this).data("messages", msgs),
...
//}}} 
		...
		// callbacks	
		$jqPm.each("onBeforeValidate,onBeforeFail,onFail,onSuccess".split(","), function(i, name) {
				
			// configuration
			if ($jqPm.isFunction(conf[name]))  {
				$jqPm(self).bind(name, conf[name]);	
			}
			
			// API methods				
			self[name] = function(fn) {
				if (fn) { $jqPm(self).bind(name, fn); }
				return self;
			};
		});	
		...
		if (conf.inputEvent) {
			inputs.bind(conf.inputEvent + ".V", function(e) {
				self.checkValidity($jqPm(this), e);
			});	
		} 
		...
		inputs.filter(":checkbox, select").filter("[required]").bind("change.V", function(e) {
			var el = $jqPm(this);
			if (this.checked || (el.is("select") && $jqPm(this).val())) {
	...
	$jqPm.fn.validator = function(conf) { 
		...
		// configuration
		conf = $jqPm.extend(true, {}, v.conf, conf);		
...
$jqPm.tools.validator.localize("fr", {
'*'		  		: 'Valeur incorrecte',
