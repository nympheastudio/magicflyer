...
class Blocktopmenu extends Module
{
        ...
        foreach ($menu_items as $item) {
            ...
            switch (substr($item, 0, strlen($value[1]))) {
                ...
                case 'CMS_CAT':
                    $category = new CMSCategory((int)$id, (int)$id_lang);
                    if (count($category)) {
                        $this->_menu .= '<li><a class="'.$category->name.'" href="'.Tools::HtmlEntitiesUTF8($category->getLink()).'" title="'.$category->name.'">'.$category->name.'</a>';
                ...
                case 'MAN':
                    ...
                    if (!is_null($manufacturer->id)) {
                        ...
                        $link = new Link;
                        $this->_menu .= '<li'.$selected.'><a  class="'.Tools::safeOutput($manufacturer->name).'" href="'.Tools::HtmlEntitiesUTF8($link->getManufacturerLink((int)$id, $manufacturer->link_rewrite)).'" title="'.Tools::safeOutput($manufacturer->name).'">'.Tools::safeOutput($manufacturer->name).'</a></li>'.PHP_EOL;
                    }
                    break;
                ...
                case 'ALLSUP':
                    $link = new Link;
                    $this->_menu .= '<li><a   class="'.$this->l('All suppliers').'" href="'.$link->getPageLink('supplier').'" title="'.$this->l('All suppliers').'">'.$this->l('All suppliers').'</a><ul>'.PHP_EOL;
                    $suppliers = Supplier::getSuppliers();
                    foreach ($suppliers as $key => $supplier) {
                        $this->_menu .= '<li><a   class="'.Tools::safeOutput($supplier['name']).'" href="'.$link->getSupplierLink((int)$supplier['id_supplier'], $supplier['link_rewrite']).'" title="'.Tools::safeOutput($supplier['name']).'">'.Tools::safeOutput($supplier['name']).'</a></li>'.PHP_EOL;
                    }
                    $this->_menu .= '</ul>';
                    break;
                ...
                case 'SUP':
                    ...
                    if (!is_null($supplier->id)) {
                        $link = new Link;
                        $this->_menu .= '<li'.$selected.'><a href="'.Tools::HtmlEntitiesUTF8($link->getSupplierLink((int)$id, $supplier->link_rewrite)).'" title="'.$supplier->name.'"  class="'.$supplier->name.'" >'.$supplier->name.'</a></li>'.PHP_EOL;
                    }
                    break;
                ...
                case 'SHOP':
                    ...
                    if (Validate::isLoadedObject($shop)) {
                        $link = new Link;
                        $this->_menu .= '<li'.$selected.'><a  class="'.$shop->name.'"  href="'.Tools::HtmlEntitiesUTF8($shop->getBaseURL()).'" title="'.$shop->name.'">'.$shop->name.'</a></li>'.PHP_EOL;
                    }
                    break;
                case 'LNK':
                    $link = MenuTopLinks::get((int)$id, (int)$id_lang, (int)$id_shop);
                    if (count($link)) {
						...
						if($id_lang!=1 && $link[0]['id_linksmenutop']==3){//no become seller btn for de and en
						
                       /* $this->_menu .= '<li><a  class="'.Tools::safeOutput($link[0]['label']).'"  href="'.Tools::HtmlEntitiesUTF8($link[0]['link']).'"'.(($link[0]['new_window']) ? ' onclick="return !window.open(this.href);"': '').' title="'.Tools::safeOutput($link[0]['label']).'">'.Tools::safeOutput($link[0]['label']).'</a></li>'.PHP_EOL;
					   */
						
						}else{
						    
							
                        $this->_menu .= '<li><a  class="'.Tools::safeOutput($link[0]['label']).'"  href="'.Tools::HtmlEntitiesUTF8($link[0]['link']).'"'.(($link[0]['new_window']) ? ' onclick="return !window.open(this.href);"': '').' title="'.Tools::safeOutput($link[0]['label']).'">'.Tools::safeOutput($link[0]['label'])./*$link[0]['id_linksmenutop'].*/'</a></li>'.PHP_EOL;
    ...
    protected function generateCategoriesMenu($categories, $is_children = 0)
    {
        ...
        foreach ($categories as $key => $category) {
            ...
            $html .= '<a href="'.$link.'" class="'.$category['name'].'" title="'.$category['name'].'">'.$category['name'].'</a>';
    ...
    protected function getCMSMenuItems($parent, $depth = 1, $id_lang = false)
    {
        ...
        if (count($categories) || count($pages)) {
            ...
            foreach ($categories as $category) {
                ...
                $this->_menu .= '<li>';
                $this->_menu .= '<a class="'.$category['name'].'"  href="'.Tools::HtmlEntitiesUTF8($cat->getLink()).'">'.$category['name'].'</a>';
            ...
            foreach ($pages as $page) {
                ...
                $this->_menu .= '<a class="'.$cms->meta_title.'"  href="'.$links[0]['link'].'">'.$cms->meta_title.'</a>';
                $this->_menu .= '</li>';
            }
